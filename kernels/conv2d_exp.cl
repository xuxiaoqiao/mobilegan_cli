  ...
  )
{
  ...
  long width_workitem_id = get_global_id(1);
  long out_channel_block_idx = get_global_id(2);
  long out_width_idx = width_workitem_id * 4;
...
__kernel void conv2d_nhwc_ver4(
  __global float* input, /* layout: HWC */
  __global float* weight, /* layout: HWOI */
  __global float* bias,
  __global float* output, /* layout: HWC */
...
{
  long size0 = get_global_size(0);
  long size1 = get_global_size(1);
  long size2 = get_global_size(2);
  long in_channel_block_range = UPDIV4(in_channel_num);
  long out_channel_block_range = (out_channel_num + 3) / 4;
  for (long out_height_idx = get_global_id(0);
       out_height_idx < out_height;
       out_height_idx += size0)
  {
    for (long width_workitem_id = get_global_id(1);
       width_workitem_id < out_width / 4;
       width_workitem_id += size1)
    {
      long out_width_idx = width_workitem_id * 4;
      for (long out_channel_block_idx = get_global_id(2);
          out_channel_block_idx < out_channel_block_range;
          out_channel_block_idx += size2)
      {
        float4 out0 = convert_float4(vload4(out_channel_block_idx, bias));
        float4 out1 = out0;
        float4 out2 = out0;
